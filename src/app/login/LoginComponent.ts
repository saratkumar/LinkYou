...
export class LoginComponent implements OnInit {
  ...
  onSubmit(type?: any) {
    ...
    this.userRequest.deviceId = this.getDeviceId();
    if(type) {
      this.loginService.signUp(this.userRequest, 123456).subscribe((data: any) => {
      ...
      this.loginService.login(this.userRequest).subscribe((data: any) => {
        if (data && data["message"] && data["message"].includes("Invalid login")) {
          this.responseMsg = "Invalid username/password";
        } else {
          this.responseMsg = "";
